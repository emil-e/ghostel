...
fn completeOverlapped(
    ...
    if (cancel) {
        if (c.CancelIoEx(handle, overlapped) == 0 and
...
pub fn write(self: *Self, data: []const u8, timeout_ms: ?u32) !backend_types.WriteResult {
    ...
    const wait_result = c.WaitForMultipleObjects(
        ...
        &handles,
        c.FALSE,
        timeout_ms orelse c.INFINITE,
    );
    const timed_out = wait_result == c.WAIT_TIMEOUT;
    const interrupted = wait_result == c.WAIT_OBJECT_0 + 1;
    if (!timed_out and !interrupted and wait_result != c.WAIT_OBJECT_0) {
