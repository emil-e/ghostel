    ...
    const written = posix.write(self.pty.primary_fd, chunk) catch |err| switch (err) {
        error.WouldBlock => {
            ...
            const timeout: i32 = if (timeout_ms) |ms|
                @intCast(@min(ms, @as(u32, @intCast(std.math.maxInt(i32)))))
            else
                -1;
            const ready = try posix.poll(&pollfds, timeout);
            if (ready == 0) return .timeout;
            if (pollfds[1].revents != 0) return .interrupted;
            const retry_written = posix.write(self.pty.primary_fd, chunk) catch |write_err| switch (write_err) {
                error.WouldBlock => return .timeout,
                else => return write_err,
            };
            if (retry_written == 0) return error.IoFailed;
            return .{ .written = retry_written };
        },
        else => return err,
    };
