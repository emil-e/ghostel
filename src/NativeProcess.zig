...
const WRITE_WAIT_TIMEOUT_MS = 20;
...
pub fn ptyWrite(self: *Self, env: emacs.Env, data: []const u8) !void {
    while (!self.write_mutex.tryLock()) {
        try env.checkQuit();
        std.Thread.sleep(WRITE_WAIT_TIMEOUT_MS * std.time.ns_per_ms);
    }
    defer self.write_mutex.unlock();

    var offset: usize = 0;
    while (offset < data.len) {
        switch (try self.ptyWriteWithTimeout(data[offset..], WRITE_WAIT_TIMEOUT_MS)) {
            .written => |n| {
                offset += n;
                try env.checkQuit();
            },
            .timeout => try env.checkQuit(),
...
pub fn ptyWriteFromTerminal(self: *Self, data: []const u8) void {
    ...
    while (offset < data.len) {
        ...
        const write_result = backend.write(data[offset..], null) catch |err| {
            log.err("Failed to write to PTY from terminal: {any}", .{err});
            return;
        };
        switch (write_result) {
            .written => |n| offset += n,
...
fn ptyWriteWithTimeout(self: *Self, data: []const u8, timeout_ms: ?u32) !backend_types.WriteResult {
    self.backend_handoff_mutex.lock();
    defer self.backend_handoff_mutex.unlock();

    if (self.backend) |*backend| return backend.write(data, timeout_ms);
