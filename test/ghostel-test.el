...
(ert-deftest ghostel-test-line-ref-stable-across-vertical-resize-underfull ()
  "Vertical shrink with an underfull viewport must not corrupt line-refs.
When the active area is not fully filled, rows_in_buffer is inflated to
size.rows via @max.  A vertical-only shrink that reduces getTotalRows()
must not be misdetected as scrollback-cap eviction and must not alter
the line-ref text properties or the line_ref_offset."
  (let ((buf (generate-new-buffer " *ghostel-test-line-ref-vresize-uf*")))
    (unwind-protect
        (with-current-buffer buf
          (let* ((term (ghostel--new 5 80 100))
                 (inhibit-read-only t))
            ;; Write only 3 lines into a 5-row terminal — viewport intentionally underfull.
            (dotimes (i 3)
              (ghostel--write-input term (format "line-%d\r\n" i)))
            (ghostel--redraw term t)
            (let ((refs-before (mapcar #'ghostel-test--line-ref '(0 1 2))))
              ;; Sanity: refs start at (0.0)..(2.0).
              (should (equal '(0 . 0) (nth 0 refs-before)))
              (should (equal '(1 . 0) (nth 1 refs-before)))
              (should (equal '(2 . 0) (nth 2 refs-before)))
              ;; Shrink viewport to exactly the number of content rows.
              ;; getTotalRows() will now equal 3, while rows_in_buffer was 5
              ;; before the fix this triggered false eviction.
              (ghostel--set-size term 3 80)
              (ghostel--redraw term)
              (dotimes (i 3)
                (should (equal (nth i refs-before) (ghostel-test--line-ref i)))))))
