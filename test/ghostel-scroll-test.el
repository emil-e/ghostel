...
(ert-deftest ghostel-test-pixel-anchor-reused-until-geometry-changes ()
  "Pixel anchoring reuses layout while terminal geometry is unchanged."
  (ghostel-test-scroll--with-buffer (buf _term 10 40 200)
    (let ((win (selected-window))
          (calls 0)
          (start-calls 0)
          (vscroll-calls 0))
      (setq-local ghostel--rendered-font 'test-font)
      (cl-letf (((symbol-value 'ghostel--pixel-anchor-supported-p) t)
                ((symbol-value 'ghostel--kitty-active) nil)
                ...
                ((symbol-function 'ghostel--pixel-anchor)
                 (lambda (_window _target)
                   (cl-incf calls)
                   (cons (point-min) 0)))
                ((symbol-function 'set-window-start)
                 (lambda (&rest _) (cl-incf start-calls)))
                ((symbol-function 'ghostel--set-window-vscroll)
                 (lambda (&rest _) (cl-incf vscroll-calls))))
        (ghostel--anchor-window win)
        (ghostel--anchor-window win)
        (should (= calls 1))
        (should (= start-calls 1))
        (should (= vscroll-calls 1))
        (setq ghostel--term-rows 11)
        (ghostel--anchor-window win)
        (should (= calls 2))
        (ghostel--anchor-window win t)
        (should (= calls 3))
        (should (= start-calls 3))
        (should (= vscroll-calls 3))))))
